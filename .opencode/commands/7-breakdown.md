...
### Starting breakdown:
1. Create session: `npx s7n session create kr-to-task <kr-uuid>`
...
- Estimate story points for each task (add after creation via `npx s7n estimate add <task-uuid> <points>`)
...
```
npx s7n task create -d "<description>" --parent <kr-uuid>
```

Then estimate each task:
```
npx s7n estimate add <task-uuid> <points>
```
...
For each dependency:
```
npx s7n task depend <dependent-task-id> <dependency-task-id>
```

Dependencies can cross KR boundaries — if a task in this KR depends on a task in another KR, still record it. Use shortIds for clarity (e.g. `npx s7n task depend O1KR2T3 O1KR1T1`).
...
```
npx s7n task comment <task-id> --type agent -m "MECE: <overlap risks, dependencies, scope boundary>"
```

Keep comments concise (1-3 sentences). Example:
```
npx s7n task comment O1KR2T1 --type agent -m "MECE: Depends on O1KR1T2 (auth system). Boundary: this task handles data model only, NOT API endpoints (see O1KR2T2). No overlap with O2 tasks."
```
